...
void lsz_fn_quick_sort(__lsz_quick_sort_t *this, int l, int r)
{
    ...
    while (1)
    {
        while ((this->fn(void_of(this->data, i, this->unit), void_of(this->data, r, this->unit)) <  0))
            i++;
        while ((this->fn(void_of(this->data, j, this->unit), void_of(this->data, r, this->unit)) >= 0) && (j >= 0))
            j--;
        if (i < j) {
